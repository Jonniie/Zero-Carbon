...
**Why?** Lagos traffic is stop-and-go. In 30 seconds, a vehicle might barely move — so GPS points cluster together and straight-line distance massively underestimates actual travel. The speedometer captures every inch of movement, even in traffic.
...
**Key finding:** The EV fleet produced only **8.6%** of the CO₂ that petrol vehicles would have emitted. Even on Nigeria's carbon-heavy grid, electric vehicles are a massive win.

---

### 2.4 Anomaly Detection

I trained an **Isolation Forest** model to flag unusual motor behavior. Here's my setup:
...
2. **Regenerative braking is invisible** — Even during confirmed braking events, the battery level still shows net consumption over 30 seconds. Why? The sampling window is too slow to catch short regen bursts, and constant loads (AC, lights, electronics) drown them out. This means the fleet's actual efficiency is better than what I measured, especially on stop-and-go routes.

3. **Energy numbers look unrealistically low** — The fleet averages ~3.8 kWh/100km. Real EVs use 15–20 kWh/100km. This happens because 30-second SoC readings miss micro-losses like battery heat and drivetrain inefficiency. The relative comparison between vehicles is still valid, but the absolute numbers shouldn't be treated as truth.
